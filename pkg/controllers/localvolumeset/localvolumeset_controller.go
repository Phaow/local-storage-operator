...
func (r *LocalVolumeSetReconciler) syncStorageClass(ctx context.Context, lvs *localv1alpha1.LocalVolumeSet) error {
	...
	// if !lvs.DeletionTimestamp.IsZero() {
	// 	return nil // Skip sync if the resource is marked for deletion
	// }
	...
	fetchStorageClass := &storagev1.StorageClass{}
	if err := r.Client.Get(ctx, client.ObjectKey{Name: lvs.Spec.StorageClassName}, fetchStorageClass); err != nil {
		if kerrors.IsNotFound(err) {
			...
			return fmt.Errorf("failed to find storageclass %q: %v", lvs.Spec.StorageClassName, err)
		}
	}

	ownerName := fetchStorageClass.Labels[common.OwnerNameLabel]
	ownerNamespace := fetchStorageClass.Labels[common.OwnerNamespaceLabel]
	_, hasOwnerKind := fetchStorageClass.Labels[common.OwnerKindLabel]

	// Only sync the miss owner kind labels storageclass used for upgrade scenario
